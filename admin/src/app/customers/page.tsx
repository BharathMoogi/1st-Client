...
export default function CustomersPage() {
  ...
  const tierColor = (t: string) => {
    ...
    if (t === "VIP Gold") return "bg-[#E0B034]/10 text-[#FFE082] border-[#E0B034]/20";
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="border-b border-white/8 pb-4"><h1 className="text-2xl font-light tracking-wide">Customers</h1><p className="text-xs text-white/40 mt-1">Registered members from Firestore profiles</p></div>
      <div className="bg-white/[0.01] border border-white/8 p-4 rounded-xl">
        <div className="relative w-72"><Search size={14} className="absolute left-3 top-3 text-white/40"/>
          <input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search customers…" className="w-full bg-white/4 border border-white/8 rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-white/20 focus:outline-none focus:border-[#E0B034]/40"/>
        </div>
      </div>
        ...
        {loading ? <div className="py-12 text-center text-white/30 text-xs animate-pulse">Loading…</div>
          ...
          : <div className="overflow-x-auto"><table className="w-full text-left text-xs font-light">
              <thead><tr className="border-b border-white/6 bg-white/[0.01] text-white/40 tracking-wider">
                <th className="py-3.5 px-6">NAME</th><th className="py-3.5 px-6">EMAIL</th><th className="py-3.5 px-6">MEMBERSHIP</th>
                <th className="py-3.5 px-6 text-right">POINTS</th><th className="py-3.5 px-6 text-right">TOTAL SPEND</th><th className="py-3.5 px-6 text-right">ORDERS</th>
              </tr></thead>
              <tbody>{filtered.length > 0 ? filtered.map((c) => (
                <tr key={c.id} className="border-b border-white/4 hover:bg-white/[0.01]">
                  ...
                  <td className="py-4 px-6"><span className={`px-2.5 py-0.5 rounded-full text-[9px] font-semibold border ${tierColor(c.tier)}`}>{c.tier||"Member"}</span></td>
                  ...
                  <td className="py-4 px-6 text-right font-mono text-white/60">{c.orders??0}</td>
                </tr>)) : <tr><td colSpan={6} className="py-12 text-center text-white/30">No customers in Firestore yet.</td></tr>}
              </tbody>
            </table></div>}
      </div>
    </div>
